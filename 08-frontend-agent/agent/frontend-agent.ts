...
${enhancedCode}`;
  ...
  private async generateFrontendTests(modifications: FieldModification[]): Promise<void> {
    ...
    let testContent = `// Generated by Frontend Agent - Tests 🧪
// Created: ${timestamp}
// Agent: agent/frontend-agent.ts

import { describe, it, expect } from "vitest";

// Import validation functions (these would be extracted from the frontend code)
`;

    // Generate test functions for each field
    for (const field of modifications) {
      if (field.action === 'add') {
        testContent += this.generateValidationFunction(field.fieldName, field.fieldType) + '\n\n';
      }
    }

    testContent += `
describe("Frontend Field Validation Tests", () => {
`;
  ...
  describe("Integration Tests", () => {
    ...
    // Add test data for each field
    for (const field of modifications) {
      if (field.action === 'add') {
        testContent += `,
        ${field.fieldName}: "${this.generateTestValue(field.fieldName, field.fieldType)}"`;
      }
    }
    ...
    testContent += `
      ...
      expect(validateUsername(validForm.username)).toBe(true);
      expect(validatePassword(validForm.password)).toBe(true);`;

    for (const field of modifications) {
      if (field.action === 'add') {
        testContent += `
      expect(validate${this.capitalize(field.fieldName)}(validForm.${field.fieldName})).toBe(true);`;
      }
    }
    ...
    const testFile = 'outputs/tests/frontend-validation.test.ts';
   ...
   */
  private generateFieldTests(field: FieldModification): string {
  ...
  describe("validate${capitalizedName}", () => {
    ...
    const validValues = this.generateValidTestValues(fieldName, field.fieldType);
    ...
    const invalidValues = this.generateInvalidTestValues(fieldName, field.fieldType);
...
`;
  ...
  /**
   * Generate valid test values for field type
   */
  private generateValidTestValues(fieldName: string, fieldType: string): string[] {
    switch (fieldType) {
      case 'email':
  ...
  /**
   * Generate invalid test values for field type
   */
  private generateInvalidTestValues(fieldName: string, fieldType: string): string[] {
    switch (fieldType) {
      case 'email':
  ...
  /**
   * Generate test value for field type
   */
